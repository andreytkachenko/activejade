...
module.exports = (function () {
    var isNodesEqual = function(node1, node2) {
        var types = [ 3, 8 ];
    ...
    var update_children = function(current, target) {
        ...
        var hasChildNode = function(elements, node) {
            for (var i = 0; i < elements.length; i++) {
                if (isNodesEqual(elements[i], node)) {
                    return true;
                }
            }
        ...
        var getNodeIndex = function (el, node) {
            ...
            while(tmp = el.childNodes[idx]) {
                if (isNodesEqual(node, tmp)) return idx;
                idx++;
            }
    ...
    var walkNodes = function (nodes, document, options, path) {
        return nodes ? flatten(nodes.map(function (i) {
            return walkNode(i, document, options, path);
        })) : [];
    }

    var walkNode = function (node, document, options, path) {
        if (!node._cache) {
            node.listen(function () {
                ...
                if (node.type === 1) {
                    ...
                    path = ancestors(node);
                    walkNode(path[0], document, options, path);
                }
            });
        }
        var update = false;
        ...
        switch (node.type) {
            ...
            case 1:
                if (!node._cache) {
                    ...
                    update_attributes(node._cache, node);
                    update_children(node._cache, walkNodes(node.children, document, options, path) );
            ...
            case 2:
                if (!node._cache || update) {
                    node._cache = walkNodes(node.children, document, options, path);
                }
            ...
            case 5:
                if (!node._cache || update) {
                    ...
                    if (typeof options.onchange === 'function') {
                        options.onchange(function (element) {
    ...
    return function (node, document, options) {
        return flatten(walkNode(node, document, options || {}, true));
    };
})();
