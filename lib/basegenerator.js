    ...
    BaseGenerator.prototype = {
        ...
        __template: function (name, dir) {
            if (this.__templates[name]) {
                ...
                return this.__templates[tpl_name];
            } else {
                throw new Error('Include Error: resource "' + name + '" not found!');
            }
        },
        ...
        i_extend: function (scope, href, origin, deps) {
            ...
            return {
                ...
                update: function () {
                    ...
                    node.setChildren(children(scope, self).children);
                }
            };
        },
        i_block: function (scope, id, embed, children, root) {
            var node = this.mCreateFragment('block');
            ...
            return {
                ...
                update: function () {
                    if (root) {
                        this.__blocks[id] = this.__blocks[id] || [];
                        this.__blocks[id].push({
                            embed: embed,
                            children: children
                        });
                    } else {
                        var blocks = (this.__blocks[id] || []).concat({
                            embed: embed,
                            children: children
                        });
                        ...
                        for (var i = 0; i < blocks.length; i++) {
                            switch (blocks[i].embed) {
                                case 'APPEND': processed.push(blocks[i].children);break;
                                case 'PREPEND': processed.unshift(blocks[i].children);break;
                                default: processed = [ blocks[i].children ];
                            }
                        }

                        node.setChildren(processed.reduce(function (a, i) {
                            return a.concat(i);
                        }, []));
                    }
                }
