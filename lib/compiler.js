...
Compiler.prototype = {
    compile: function (source, name, compile_type, callback_name) {
        ...
        var template_f = util.format(
                        'function(s,d){var n_0,__FILE__="%s";%s;n_0=d.generate([n_%s]);return n_0;}',
                        name, items.join(''), ids.join(',n_'));

        switch (compile_type) {
            case 'callback':
                return util.format('%s("%s",%s);', callback_name, name, template_f);
            case 'common':
                return util.format('module.exports={FILE_NAME:"%s",template:%s};', name, template_f);
            case 'amd':
    ...
    walkIdentifier: function (node) {
        var excludes = ['this', 'JSON', 'Date', 'decondeURIComponent', 'encodeURIComponent',
                        'Promise'];
        return {
            value: (~excludes.indexOf(node.id) || node.id.charAt(0) === '$') ? 'c.' + node.id : 's.' + node.id,
            deps: [node.id]
        };
    },
