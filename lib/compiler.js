...
Compiler.prototype = {
    compile: function (source, name) {
        ...
        return util.format(
                        'function(s,d){var n_0,__FILE__="%s";%s;n_0=d.generate([n_%s]);return n_0;}',
    ...
    walkNode: function (node, notwrap) {
        ...
        res.value = (!res.writable || notwrap) ? res.value : 'd.__p(' + res.value + ')';
    ...
    walkCallExpression: function (node) {
        ...
        return {
            value: 'd.__p((' + callee.value + ').call('+[callee.parent||'null'].concat(args.values).join(',') + '))',
    ...
    walkAssignmentExpression: function (node) {
        ...
        if (node.operator === '=') {
            ...
            value = left.value + '=d.__p(' + left.value + ')' + node.operator.substr(0, node.operator.length-1) + right.value
        }
    ...
    walkWhile: function (node, parent_id) {
        ...
        return [id, util.format('var n_%d=d.next("dowhile",[s,%s,%s,%s,[%s]]);', id, parent_js, block_js, expr_js, __deps(expr.deps))];
    },
    ...
    walkForInIf: function (node, parent_id) {
        ...
        return [id, util.format('var n_%d=d.next("forin",[s,%s,%s,%s,"%s","%s",%s,[%s]]);',
    ...
    walkIfElse: function (node, parent_id) {
        ...
        return [id, util.format('var n_%d=d.next("ifelse",[s,%s,%s,%s,[%s]]);', id, parent_js, children_js, cond_js, __deps(cond.deps))];
    },
    ...
    walkCase: function (node, parent_id) {
        ...
        return [
            id,
            util.format('var n_%d=d.next("casewhen",[s,%s,[%s],[%s],%s,%s,[%s]]);', id, parent_js, children_js.join(','), exprs.join(','), expr_value, default_idx, __deps(deps))
        ];
    },
    ...
    walkInclude: function (node, parent_id) {
        ...
        return [id, util.format('var n_%d=d.next("include",[s,%s,%s,%s,__FILE__,[%s]]);', id, parent_js, href_js, with_js, __deps(href.deps))];
    },
    ...
    walkTag: function (node, parent_id) {
        ...
        return [id, util.format('var n_%d=d.next("tag",[s,%s,%s,%s,%s,[%s],[%s]]);', id, parent_js, children_js, tag_js, attrs.value, decors_js, __deps(attrs.deps))];
    },
...
function __wrap(value) {
    return 'function(s,c){return ' + value + ';}';
}
